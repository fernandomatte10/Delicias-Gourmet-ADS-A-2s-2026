...
Para criar o repositório local e fazer o primeiro commit:

```bash
git init
git add .
git commit -m "feat: adiciona site Delícias Gourmet"
git branch -M main
git remote add origin SEU_LINK_DO_GITHUB
git push -u origin main
```

> O arquivo `data/pedidos.json` não é enviado para o GitHub porque pode conter dados de clientes. Em cada ambiente, ele será criado automaticamente pelo servidor quando necessário.
